document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 1. Capturamos también la 'description' de la base de datos
    async function fetchProducts() {
        try {
            ...
            COMPONENTS_DATA = response.data.map(product => ({
                ...
                price: product.price,
                image: product.image || '',
                description: product.description || 'Sin descripción detallada disponible.'
            }));
            renderComponents();
        } catch (error) {
    ...
    function renderCartItems() {
        ...
        carrito.forEach(item => {
            ...
            <div class="flex items-center space-x-3 w-3/4 min-w-0">
                <img src="${imageUrl}" alt="${item.name}" class="w-12 h-8 object-cover rounded flex-shrink-0" />
                <div class="flex flex-col min-w-0">
                    <span class="text-sm font-medium text-gray-800 truncate block" title="${item.name}">
                        ${item.name}
                    </span>
    ...
    // --- SOLUCIÓN DEFINITIVA A LAS TARJETAS QUE SE ROMPEN ---
    function renderComponents() {
        ...
        COMPONENTS_DATA.forEach(component => {
            ...
            productCard.className = 'card-bg rounded-xl shadow-lg p-4 flex flex-col justify-between transform transition duration-500 hover:scale-[1.02] overflow-hidden';
            ...
            <div class="flex flex-col items-center w-full overflow-hidden">
                ...
                <h3 class="text-lg font-bold text-gray-800 text-center w-full truncate px-4" title="${component.name}">
                    ${component.name}
                </h3>
